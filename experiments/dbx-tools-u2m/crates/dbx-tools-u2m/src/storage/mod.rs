...
pub async fn open_store(options: StoreOptions) -> Result<Arc<dyn CredentialStore>> {
    ...
    match backend {
        ...
        StoreBackend::Keyring => open_keyring(cache_dir).await,
        StoreBackend::Auto => {
            #[cfg(feature = "keyring")]
            {
                if let Ok(store) = KeyringStore::probe(cache_dir.clone()).await {
...
fn default_cache_dir() -> PathBuf {
    UserDirs::new()
        ...
        .unwrap_or_else(|| PathBuf::from(".dbx-tools-u2m"))
}
...
async fn open_keyring(cache_dir: PathBuf) -> Result<Arc<dyn CredentialStore>> {
    #[cfg(feature = "keyring")]
    {
        Ok(Arc::new(KeyringStore::probe(cache_dir).await?))
    }
