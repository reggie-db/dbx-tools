...
function fakeClient(overrides: Record<string, unknown> = {}) {
  ...
  const client = {
    config: {
      getHost: async () => "https://example.databricks.com",
      authenticate: async (headers: Headers) => headers.set("authorization", "Bearer test"),
    },
    vectorSearchIndexes: {
...
describe("search index lifecycle", () => {
  // createIndex / provision embed a probe (and seed rows) via the serving
  // endpoint. Stub fetch to return a fixed 1024-dim embedding per input so the
  // lifecycle asserts run with no workspace.
  const realFetch = globalThis.fetch;
  beforeEach(() => {
    globalThis.fetch = (async (_url: unknown, init?: { body?: string }) => {
      const body = init?.body ? (JSON.parse(init.body) as { input: string[] }) : { input: [] };
      const data = body.input.map(() => ({ embedding: Array.from({ length: 1024 }, () => 0.1) }));
  ...
  it("creates a self-managed direct-access index by default (auto-resolving the dimension)", async () => {
    // No sourceTable and no explicit dimension: the client creates a
    // DIRECT_ACCESS index with self-managed vectors, probing the embedding
    // endpoint (stubbed above to 1024 dims) to size the vector column. Managed
    // embeddings are Delta-Sync only, so there are no embedding_source_columns.
    const fake = fakeClient();
    ...
    assert.equal(create.index_type, "DIRECT_ACCESS");
    const spec = create.direct_access_index_spec as Record<string, unknown>;
    assert.equal(spec.embedding_source_columns, undefined);
    const cols = spec.embedding_vector_columns as Array<Record<string, unknown>>;
    assert.equal(cols[0].name, "embedding");
    assert.equal(cols[0].embedding_dimension, 1024);
    ...
    assert.equal(schema.embedding, "array<float>");
  });
