...
await polygotTest(
  ...
  (implementation, language) => {
    ...
    const basicCapabilities = (endpoint: Parameters<typeof endpointCapabilities>[0]) => {
      const { chat, embedding, tools } = endpointCapabilities(endpoint);
      return { chat, embedding, tools };
    };

    describe(`versionTuple (${language})`, () => {
    ...
    describe(`endpointCapabilities (${language})`, () => {
      it("reads capability off the task hint", () => {
        assert.deepEqual(basicCapabilities(chat("databricks-gpt-5-5")), {
          chat: true,
          embedding: false,
          tools: true,
        });
        assert.deepEqual(basicCapabilities(embedding("databricks-gte-large-en")), {
          chat: false,
          embedding: true,
          tools: false,
        });
      ...
      it("falls back to the classified class when the endpoint carries no task", () => {
        ...
        assert.equal(
          endpointCapabilities({ name: "custom", class: ModelClass.ChatFast }).tools,
          false,
        );
        assert.equal(
      ...
      it("keeps an unclassified, untasked endpoint out of both buckets", () => {
        assert.deepEqual(basicCapabilities({ name: "mystery" }), {
          chat: false,
          embedding: false,
          tools: false,
        });
      ...
      it("lets embedding win when task and class disagree", () => {
        ...
        assert.deepEqual(basicCapabilities(conflicted), {
          chat: false,
          embedding: true,
          tools: false,
        });
    ...
    describe(`supportsToolsByFamily (${language})`, () => {
    ...
    describe(`classifyEndpoints (${language})`, () => {
      ...
      it("places unscored but recognizable chat endpoints by family heuristic", () => {
        ...
        assert.deepEqual(classNames(buckets, ModelClass.ChatFast), ["databricks-claude-haiku-3-5"]);
      });
    ...
    describe(`classifyByFamily (${language})`, () => {
      it("maps well-known variants to chat classes and returns null for unknowns", () => {
        assert.equal(
          classifyByFamily("databricks-claude-opus-4-8")?.class,
          ModelClass.ChatThinking,
        );
        assert.equal(
          classifyByFamily("databricks-claude-sonnet-4-6")?.class,
