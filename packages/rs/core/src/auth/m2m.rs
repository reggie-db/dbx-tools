...
mod tests {
    ...
    use super::*;
    use crate::{
        AuthKind, AuthOptions, AuthSession, DatabricksAuthClient, MemoryStore, TargetKind,
    };

    #[tokio::test]
    async fn m2m_and_app_sp_share_client_credentials_semantics() {
        let (host, server) = start_server(2).await;
        for auth_kind in [AuthKind::MachineToMachine, AuthKind::AppServicePrincipal] {
            let profile = Profile {
                name: "service".into(),
