 ...
 * {@link AiSearchPluginConfig} (the plugin's slice of AppKit config), the JSON
 * Schema the manifest publishes for it, and {@link resolveAiSearchConfig}
 * which layers that config over environment defaults into the concrete
 * {@link ResolvedAiSearchConfig} the runtime, client, tools, and routes read.
 *
 ...
 * `AI_SEARCH_INDEX`, `AI_SEARCH_ENDPOINT`, `AI_SEARCH_COLUMNS`,
 * `AI_SEARCH_PAGE_SIZE`, `AI_SEARCH_MODE`, `AI_SEARCH_EMBEDDING_MODEL`,
 * `AI_SEARCH_TIMEOUT_MS`, `AI_SEARCH_WRITE`.
 *
 * @module
 */
...
export const INDEX_ENV = "AI_SEARCH_INDEX";
...
export const ENDPOINT_ENV = "AI_SEARCH_ENDPOINT";
...
export const DEFAULT_BASE_PATH = "/api/ai-search";
 ...
 */
export interface AiSearchIndexConfig {
 ...
 */
export interface AiSearchPluginConfig extends BasePluginConfig {
  ...
  indexes?: Array<string | AiSearchIndexConfig>;
...
export interface ResolvedAiSearchConfig {
  defaultIndex?: string;
...
export const AI_SEARCH_CONFIG_SCHEMA: JSONSchema7 = {
  type: "object",
...
function toResolvedIndex(entry: string | AiSearchIndexConfig): ResolvedIndexConfig | null {
...
function resolveMode(configured: SearchMode | undefined): SearchMode {
  const raw = configured ?? string.trimToNull(process.env.AI_SEARCH_MODE) ?? undefined;
  if (raw === undefined) return DEFAULT_MODE;
  if (raw !== "hybrid" && raw !== "vector" && raw !== "keyword") {
    throw new ConfigurationError(
      'mode must be "hybrid", "vector" or "keyword" (env: AI_SEARCH_MODE)',
      { context: { field: "mode", envVar: "AI_SEARCH_MODE" } },
    );
  }
  return raw;
}
 ...
 * {@link ResolvedAiSearchConfig}. The default index is unioned into the known
 ...
 */
export function resolveAiSearchConfig(config: AiSearchPluginConfig = {}): ResolvedAiSearchConfig {
  ...
  if (defaultIndexRaw && !configured.some((i) => i.name === defaultIndexRaw)) {
    const fromEnv = string.parseList(process.env.AI_SEARCH_COLUMNS);
    configured.unshift({
  ...
  const columns = config.columns ?? string.parseList(process.env.AI_SEARCH_COLUMNS);
  ...
  return {
    ...
    pageSize: resolvePositiveInt(config.pageSize, "AI_SEARCH_PAGE_SIZE", DEFAULT_PAGE_SIZE),
    mode: resolveMode(config.mode),
    ...((string.trimToNull(config.embeddingModel) ??
    string.trimToNull(process.env.AI_SEARCH_EMBEDDING_MODEL))
      ? {
          embeddingModel:
            string.trimToNull(config.embeddingModel) ??
            string.trimToNull(process.env.AI_SEARCH_EMBEDDING_MODEL) ??
