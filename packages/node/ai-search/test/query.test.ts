...
import { defaultAlias, resolveAiSearchConfig, resolveIndexName } from "../src/config.ts";
import {
  compileFilter,
  toDocumentArray,
  toHits,
...
describe("ai-search query translation", () => {
...
describe("ai-search config", () => {
  ...
  it("unions the default index into the known set and resolves it by alias", () => {
    const config = resolveAiSearchConfig({ index: "main.support.docs" });
  ...
  it("defaults mode, page size, and base path", () => {
    ...
    assert.equal(config.basePath, "/api/ai-search");
