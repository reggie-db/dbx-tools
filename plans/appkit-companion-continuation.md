...
browser-safe contracts in `packages/shared`, Node/AppKit integrations in
`packages/node`, CLIs in `packages/cli`, and React/Vite foundations in
`packages/ui`.
...
1. **Leverage projen + what's already in this repo.** Don't hand-roll project
   ...
   package is just a `src/`-bearing folder under `packages/`; projen
...
- **shared-core** (`packages/shared/core` → `@dbx-tools/shared-core`):
...
- **node-core** (`packages/node/core` → `@dbx-tools/node-core`): the Node-only
  ...
  `packages/node/`. Anything needing `child_process`/`fs`/`process` depends on
...
- **`packages/node/` = Node-tagged tier, `packages/shared/` = browser-safe.**
  A package's folder path drives its tag: put anything that touches `node:*` /
  `child_process` / a Node-only dep under `packages/node/` (auto-tags `node`).
...
| `0616cd7`            | **Move the path toolkit under `packages/node/`**. The current package is `@dbx-tools/node-path`, covering find, match, ignore, scan, and watch helpers.                                                                                                                                                                                                                                                                                    |
| `f660b77`            | **shared-core is a universal base dep** (added to every package via the blanket mixin, any tag) + **move `projen` engine under `packages/node/`** (`@dbx-tools/projen`, `node`-tagged by path).                                                                                                                                                                                                                                            |
...
| `45ab168`            | **Port `model-proxy` → `@dbx-tools/model-proxy`** (`packages/cli/model-proxy`, `cli`-tagged, ships the `model-proxy` bin). See "model-proxy" below.                                                                                                                                                                                                                                                                                        |
...
| (pending commit)     | **Rename `node-file-scan` → `@dbx-tools/node-path`** (`packages/node/path`). It's the path toolkit (find/match/ignore/scan/watch), not just file matching. Only the projen engine consumed it.                                                                                                                                                                                                                                             |
...
- `packages/node/projen/src/codegen.ts` — `generateCodegen()`. Scans
...
dashboards `.d.ts` into `packages/shared/sdk-model/src/dashboards.ts`
...
- `packages/shared/sdk-model` → `@dbx-tools/shared-sdk-model`, tag
...
- **`exec` + `project` → `@dbx-tools/node-core`** (`packages/node/core`,
...
Implementation: `packages/node/projen/src/module-exports.ts` extracts a
...
`@dbx-tools/node-appkit` (`packages/node/appkit`, `node`-tagged) — the base for
...
Ported `original genie` → `@dbx-tools/node-genie` (`packages/node/genie`,
...
Ported `original model` → `@dbx-tools/node-model` (`packages/node/model`,
...
Ported `original model-proxy` → `@dbx-tools/model-proxy` (`packages/cli/model-proxy`,
...
(`packages/shared/email`, `shared`-tagged, zod-only, browser-safe). Renamed
...
Ported `original appkit-email` → `@dbx-tools/node-email` (`packages/node/email`,
...
New **`@dbx-tools/node-databricks`** (`packages/node/databricks`, `node`-tagged)
...
- **React UI packages** (`ui-appkit`, email UI, Mastra UI) —
  ...
  Scope each package individually and keep shared contracts in `packages/shared`.
...
(cd packages/shared/<pkg> && pnpm exec projen compile)   # type-check
(cd packages/shared/<pkg> && pnpm exec projen test)      # node:test
