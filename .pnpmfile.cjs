/**
 * Link the projen engine (`@dbx-tools/projen`) from the in-repo `dev-projen/`
 ...
 * lives in the standalone `dev-projen/` project, which is deliberately NOT a
 * member of this workspace. This `readPackage` hook rewrites any
 * `@dbx-tools/projen` dependency to a local `link:` so the root synth consumes
...
const ENGINE_LINK = `link:${path.resolve(__dirname, "dev-projen")}`;
