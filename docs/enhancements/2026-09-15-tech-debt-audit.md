...
- Published npm and Python packages now share the Apache-2.0 policy already used
  by Rust. All 40 public npm manifests have descriptions, and generated npm and
  Python package trees carry license files. Package homepages remain deferred as
  the sole Low metadata finding.
- The published Tunnel declaration surface now declares the Express type
  dependency required by its public middleware signatures.
...
| DEP-01  | Dependency debt           | `.projenrc.ts:202`                                                                                                                | Medium   | S      | Completed 2026-09-15. A blanket rule previously added `@dbx-tools/shared-core` to every JavaScript package, including nine published packages with no direct import.                                                                                     | The blanket rule is replaced by an explicit list of production-source importers. The nine unused dependencies are gone, and all nine affected packages compile independently.                                        |
...
| DOC-08  | Source documentation      | `packages/js/shared/mastra/src/wire.ts:79`                                                                                        | Medium   | M      | Completed 2026-09-15. All 30 previously undocumented `@dbx-tools/shared-mastra` symbols now carry direct JSDoc at their owning declarations.                                                                                                             | Client config, history, threads, feedback, charts, statement embeds, lifecycle events, and final Genie result types now document ownership, optional fields, and compatibility behavior.                             |
| DOC-09  | Source documentation      | `packages/js/shared/genie/src/genie-model.ts:64`                                                                                  | Medium   | M      | Completed 2026-09-15. All 24 previously undocumented `@dbx-tools/shared-genie` symbols now carry direct JSDoc, including the generated status-schema export.                                                                                             | Attachment variants, message projections, event ordering, terminal states, and every public event alias now document their wire and stream semantics at the source declaration.                                      |
| DOC-10  | Source documentation      | `packages/js/shared/core/src/async.ts:61`                                                                                         | Medium   | M      | Completed 2026-09-15. All 16 previously undocumented `@dbx-tools/shared-core` symbols now carry direct JSDoc.                                                                                                                                            | Polling, memoization, logging, branding, collection, and identifier contracts now state defaults, cancellation, failure, and overflow behavior without documenting private implementation details.                   |
...
| DRY-02  | Request context           | `packages/js/node/appkit-mastra/src/_agent-route-context.ts:1`                                                                    | Medium   | S      | Completed 2026-09-15. History and thread routes previously repeated fixed/dynamic agent validation, lookup, request-context reads, resource errors, and JSON responses.                                                                                  | One private resolver now owns route binding and caller context, with an explicit required/optional thread-id policy reused by both route modules.                                                                    |
| DRY-03  | Search policy             | `packages/js/node/search/src/_search-options.ts:1`                                                                                | Medium   | S      | Completed 2026-09-15. Plugin exports and Mastra tools previously rebuilt single-index and federated client options independently.                                                                                                                        | One private mapper now translates validated shared requests into `SearchClient` options for both adapters, including cancellation and zero score thresholds.                                                         |
| DRY-04  | Runtime lifecycle         | `packages/rs/core/src/runtime.rs:33`                                                                                              | Medium   | S      | Completed 2026-09-15. Both proxy binaries previously carried the same Ctrl-C and Unix SIGTERM future.                                                                                                                                                    | Rust core now owns the documented shutdown future; model proxy and Lakebase proxy import it, and only core enables Tokio's signal feature.                                                                           |
| DRY-05  | CLI forwarding            | `packages/py/graphiti/src/dbx_tools/graphiti/_cli.py:1`                                                                           | Medium   | S      | Completed 2026-09-15. The Graphiti foreground and supervisor entry points previously duplicated Cyclopts parsing, `--` splitting, execution, and exit translation.                                                                                       | One private forwarding runner now accepts the app, optional default command, and command-specific forwarded-argument binder.                                                                                         |
...
| DRY-07  | Release tooling           | `projen/src/_release-platform.ts:1`                                                                                               | Medium   | S      | Completed 2026-09-15. Bump and release-PR commands previously duplicated release levels, platform constants, selector types, and repeatable Commander option parsers.                                                                                    | The existing release-platform module now owns the vocabulary, types, option builders, collection behavior, and Cartesian filter used by both commands.                                                               |
...
- [x] `DOC-01`, `DOC-02`, `DOC-03`, `DOC-04`, `DOC-07`, and `DOC-11` completed
      ...
      validation.
- [x] `DEP-01`, `META-01`, `META-02`, and `META-04` completed on 2026-09-15.
      `shared-core` is now declared only by production-source importers; the nine
      unused published dependencies are removed. One Apache-2.0 constant drives
      JavaScript, Python, and Rust metadata, all 40 public npm packages have
...
- [x] `DRY-02`, `DRY-03`, `DRY-04`, `DRY-05`, and `DRY-07` completed on
      2026-09-15. Mastra routes, Search adapters, Rust proxy shutdown, Graphiti
      entry points, and release commands now each use one private or shared policy
      owner without widening JavaScript package barrels. Focused validation passed:
      AppKit-Mastra and Search compilation with 17 tests, Projen compilation with
      seven tests, all 26 Graphiti unit tests, Rust checks across core and both
      proxies, four core runtime tests, formatting, and the source-doc ratchet.
- [x] `DOC-08`, `DOC-09`, and `DOC-10` completed on 2026-09-15. Direct JSDoc now
      covers all 70 previously undocumented public declarations across shared
      Mastra, Genie, and core. The repository baseline fell from 183 to 113 with
      zero remaining entries for those packages. Focused validation passed: all
      three package compiles, Prettier, and the source-doc ratchet.
- [x] Complete every Medium finding in focused batches. Low findings remain
...
5. [x] Document Python Postgres while excluding generated bindings and intentional
       compatibility aliases, then clear the shared Mastra, Genie, and core
       package backlogs at their owning declarations.
...
- [x] Move release OS, architecture, level, and repeatable option helpers into
      the existing release-platform module.
- [x] Extract the shared Rust shutdown signal future.
- [x] Extract the Graphiti `--` forwarding runner.
