...
def channel_name(channel: object = _DEFAULT_CHANNEL) -> str:
    ...
    stable = "\0".join(_stable_key(part) for part in parts)
    suffix = _fnv_hash(stable, _CHANNEL_HASH_LENGTH)
    labels = [part for part in parts if isinstance(part, (str, int, float, bool))]
    body = _identifier(*labels)[: _MAX_CHANNEL_LENGTH - len(suffix) - 1].rstrip("_")
...
def _stable_key(value: object, seen: set[int] | None = None) -> str:
    if value is None:
        return "null"
    ...
    try:
        if isinstance(value, (list, tuple)):
            return f"array:[{','.join(_stable_key(item, seen) for item in value)}]"
        if isinstance(value, (set, frozenset)):
            return f"set:[{','.join(sorted(_stable_key(item, seen) for item in value))}]"
        if isinstance(value, Mapping):
            entries = sorted(
                f"{_stable_key(key, seen)}={_stable_key(item, seen)}" for key, item in value.items()
            )
...
def _fnv_hash(value: str, length: int) -> str:
    digest = 0x811C9DC5
    tokens = ["[", "string:", value, ",", "]"]
    for token in tokens:
    ...
    remaining = digest
    if remaining == 0:
        encoded_digest = _BASE32_ALPHABET[0]
    while remaining:
        encoded_digest = _BASE32_ALPHABET[remaining & 31] + encoded_digest
        remaining >>= 5
    return encoded_digest.rjust(7, _BASE32_ALPHABET[0])[: min(length, 7)]
