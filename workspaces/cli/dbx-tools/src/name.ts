...
const NAME_PARTS_REGEXP =
    /(?<=[a-z0-9])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])|[^A-Za-z0-9._-]+/g;

const NAME_PARTS_EDGE_REGEXP =
...
class PackageIdentifierImpl {
    ...
    constructor(scope: string | null | undefined, public readonly name: string) {
...
function parsePackageIdentifier(value: string): PackageIdentifier | undefined {
    ...
    if (match) {
        const [, ...parts] = match;
        const [first, second] = parts.map((part) => toNameParts(part)).filter(parts => parts.length).map(parts => parts.join("-"));
        if (second) {
...
export function toNameParts(
    value: string | null | undefined
): string[] {
    if (!value) return [];
    return value.split(NAME_PARTS_REGEXP)
        .map((part) =>
            part
                .replace(NAME_PARTS_EDGE_REGEXP, "")
                .toLowerCase()
        )
        .filter(Boolean)
}
