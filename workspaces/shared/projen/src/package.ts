/**
 * Workspace package tooling: naming, guards, manifest fields, and the projen
 * Node/TypeScript project classes plus the shared root init.
 *
 * {@link DBXToolsNodeProject} (monorepo root) and {@link DBXToolsTypeScriptProject}
 * (a package, or a standalone compiling root) implement {@link IDBXToolsNodeProject}.
 */
...
/** A dbx-tools package backed by projen's Node toolchain. */
export interface IDBXToolsNodeProject extends IDBXToolsProject, javascript.NodeProject {
...
export class PackageIdentifier {
   ...
   */
  static of(name: string, ...names: string[]): PackageIdentifier {
...
export function isDBXToolsPackage(c: IConstruct): c is IDBXToolsNodeProject {
  return c instanceof javascript.NodeProject && "dbxToolsConfig" in c && "scope" in c;
}

/** Mixin predicate: package carries `tag`. */
export function packageWithTag(
  tag: string,
): (construct: IConstruct) => construct is IDBXToolsNodeProject {
  return (construct): construct is IDBXToolsNodeProject =>
    isDBXToolsPackage(construct) && construct.dbxToolsConfig.tags.includes(tag);
}
 ...
 */
export class DBXToolsNodeProject extends javascript.NodeProject implements IDBXToolsNodeProject {
  readonly scope: string;
...
export class DBXToolsTypeScriptProject
  ...
  implements IDBXToolsNodeProject {
  readonly scope: string;
  readonly dbxToolsConfig: DBXToolsConfig;
  pnpmWorkspace?: DBXToolsPNPMWorkspace;
  rootTsconfig?: DBXToolsRootTsconfig;
...
function initProject(
  ...
  const existing = new Map<string, IDBXToolsNodeProject>();
