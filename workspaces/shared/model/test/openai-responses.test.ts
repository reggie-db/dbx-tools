...
describe("createResponsesStreamTranslator", () => {
  ...
  it("assembles argument fragments into one function_call item", () => {
    ...
    translator.feed({
      choices: [
        { delta: { tool_calls: [{ index: 0, id: "c1", function: { name: "lookup" } }] } },
      ],
    });
    translator.feed({ choices: [{ delta: { tool_calls: [{ index: 0, function: { arguments: '{"q' } }] } }] });
    translator.feed({ choices: [{ delta: { tool_calls: [{ index: 0, function: { arguments: '":1}' } }] } }] });
