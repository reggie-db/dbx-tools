...
- `packages/rs/model-proxy` is the public `dbx-model-proxy` Rust binary that
  ...
  Anthropic base64 shapes. Resize images proportionally when either edge exceeds
  1,568 pixels or the image exceeds 1.15 megapixels. Do not fetch remote image
  URLs. The local token
...
- `packages/js/node/appkit-graphiti` — AppKit process plugin for the Python
  ...
  `concurrently`; the Python launcher runs Graphiti and the model proxy under Honcho.
...
- `packages/py/graphiti` — native local launcher for upstream Graphiti's MCP
  ...
  Run Graphiti and the managed model proxy under Honcho so it forwards shutdown signals
  ...
  1024-dimensional GTE embedding endpoint, and let an explicit model-proxy URL
  disable proxy ownership. Managed mode passes an optional profile override or
  `DATABRICKS_CONFIG_PROFILE` to `dbx-model-proxy`; the Rust Databricks client
...
- **Python workspace roots** — `DBXToolsPythonWorkspace.root` defaults to
  ...
  `pythonRequires` value. The shared range is Python 3.11 through the Python 3
  release line. The root
  ...
  index. The root workspace depends on `dbx-tools-graphiti` so
  `uv run dbx-graphiti` is available from the repository root.
  ...
  the ones the caller wanted anyway; when they are not, keep pure functions
  such as Postgres address normalization importable without constructing a
  `WorkspaceClient`.
...
- `@dbx-tools/appkit-graphiti`: AppKit has no Graphiti or embedded MCP sidecar
  ...
  existing public port. The Python package owns Graphiti, Neo4j, and the model proxy;
...
and stop. Do not retain a Python port after its final runtime consumer moves to
the owning Rust binary or binding.
...
- `dbx_tools.core.cache` - platform cache root resolution, cross-platform
  process and thread file locking, and the shared `check_lock_check` refresh
  sequence;
