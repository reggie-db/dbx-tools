...
- `packages/py/model` — Python Model Serving invocation, endpoint listing,
  ...
  established sorting-version parser. `model_status` refreshes the Databricks
  Foundation Model retirement page through a daily disk cache guarded by the
  shared core file lock, keeps the parsed result in memory for the same TTL,
  and falls back to the committed `_retired_models.py` snapshot generated by a
  full projen post-synth through the existing `dbx-litellm retired-models`
  command. Do not add a package-local command entry point to `model_status.py`.
  A refresh failure is cached for the day and logged without a traceback; the
  cache file retains the full diagnostic. Endpoint
  discovery gets one `ModelStatus` from `model_status.get()` so future status
