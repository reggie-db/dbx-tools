...
function thought(thought_type: GenieThought["thought_type"], content: string): GenieThought {
...
describe("eventDetector", () => {
  it("returns an EventDetector with the literal type and the detect fn", () => {
    const fn: Parameters<typeof eventDetector<"status">>[1] = (current, _previous, space_id) => ({
...
describe("detectStatus", () => {
  it("emits with previous_status undefined on the first status seen", () => {
    const out = detectStatus.detect(makeMessage({ status: "SUBMITTED" }), undefined, SPACE_ID);
    equalPayload(out, {
      status: "SUBMITTED",
      previous_status: undefined,
      space_id: SPACE_ID,
...
describe("detectAttachmentAdded", () => {
  it("emits on first sight (no previous) with the detected attachment_type", () => {
    ...
    const out = detectAttachmentAdded.detect(att, undefined, makeLoc({ attachment_id: "a1" }), 0);
    equalPayload(out, {
...
describe("detectThinking", () => {
  ...
  it("emits only the newly-added (type, content) tuples on a subsequent snapshot", () => {
    ...
    equalPayload(out, [{ ...makeLoc(), text: "second", thought_type: "THOUGHT_TYPE_STEPS" }]);
  });
  ...
  it("dedupes within a single snapshot if Genie ever ships the same tuple twice", () => {
    const att: GenieAttachment = {
      attachment_id: "q1",
      query: {
        thoughts: [thought("THOUGHT_TYPE_STEPS", "step"), thought("THOUGHT_TYPE_STEPS", "step")],
      },
    };
    const out = detectThinking.detect(att, undefined, makeLoc(), 0);
    equalPayload(out, [{ ...makeLoc(), text: "step", thought_type: "THOUGHT_TYPE_STEPS" }]);
  });
});
...
describe("detectText", () => {
  ...
  it("does not emit when text is undefined", () => {
    assert.equal(detectText.detect({ attachment_id: "x" }, undefined, makeLoc(), 0), undefined);
  });
});
...
describe("eventsFromMessage", () => {
  ...
  it("fans out per-attachment events with the correct index", () => {
    ...
    const attachmentEvents = events.filter((e): e is AttachmentEvent => e.type === "attachment");
